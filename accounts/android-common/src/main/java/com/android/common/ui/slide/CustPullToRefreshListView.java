...
import com.android.common.ui.pulltorefresh.PullToRefreshListView;


/**
 * Created by chengyuchun on 2016/7/18 0018.
 */
public class CustPullToRefreshListView extends PullToRefreshListView {
    ...
    public CustPullToRefreshListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CustPullToRefreshListView(Context context, Mode mode) {
        super(context, mode);
    }

    public CustPullToRefreshListView(Context context, Mode mode, AnimationStyle style) {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            /**
             * Created by chengyuchun on 2016/5/16 0016 15:11
             * 处理与 viewPager的冲突
             */
            if (Math.abs(ev.getX() - mX) > Math.abs(ev.getY() - mY)) {
                needConsumeTouch = false;
                getParent().requestDisallowInterceptTouchEvent(false); //交给父控件处理
                return false;
            }
        }
    ...
    private boolean isAtTop() {
        ...
        int childNum = getRefreshableView().getChildCount();
        if (childNum == 0) {
            // 没有child，肯定在顶部
            resultValue = true;
        } else {
